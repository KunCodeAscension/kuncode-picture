package com.kuncode.kuncodepicturebackend.manager.sharding;
...
public class DynamicShardingManager {
    ...
    private static final ConcurrentHashSet<String> availableTargetNames = new ConcurrentHashSet<>();

    @PostConstruct
    public void initialize() {
        log.info("初始化动态分表配置...");
        setShardingTableNodes();
    }
    ...
    public void updateShardingTableNodes(Long spaceId) {
        availableTargetNames.add(LOGIC_TABLE_NAME + "_" + spaceId);
        ShardingTableNodes(availableTargetNames);
    }

    /**
     * 更新 ShardingSphere 的 actual-data-nodes 动态表名配置
     */
    private void setShardingTableNodes() {
        Set<String> tableNames = fetchAllPictureTableNames();
        ShardingTableNodes(tableNames);
    }

    public void ShardingTableNodes(Set<String> tableNames){
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> "kuncode_picture." + tableName) // 确保前缀合法
    ...
    public void createSpacePictureTable(Space space) {
        ...
        if (space.getSpaceType() == SpaceTypeEnum.TEAM.getValue() && space.getSpaceLevel() == SpaceLevelEnum.FLAGSHIP.getValue()) {
            ...
            try {
                ...
                // 更新分表
                updateShardingTableNodes(spaceId);
            } catch (Exception e) {
                log.error("创建图片空间分表失败，空间 id = {}", space.getId());
                throw new BusinessException(ErrorCode.OPERATION_ERROR,"旗舰版图片空间存储创建异常");
            }
        }
    }
