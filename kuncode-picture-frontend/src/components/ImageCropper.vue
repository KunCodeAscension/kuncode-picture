...
const handleImgMoving = (params) => {
  const { moving, axis } = params;
  console.log("图片移动状态：", moving ? "移动中" : "已停止");
  console.log("图片坐标：", axis); // {x1, y1, x2, y2}
  console.log(cropperRef.value.getImgAxis())
};

const handleCropMoving = (params) => {
  const { moving, axis } = params;
  console.log("裁剪框：", moving ? "移动中" : "已停止");
  console.log("裁剪框坐标：", axis); // {x1, y1, x2, y2}
  console.log(cropperRef.value.cropW)
}
